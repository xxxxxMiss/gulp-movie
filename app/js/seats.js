...
! function(pui) {
	...
	function Seats(options) {
		...
		for (var key in data) { // 遍历行
			...
			for (var i = 0, len = row.length; i < len; i++) { // 遍历列
				var col = null;
				if ((row[i].seatType == 0 || row[i].seatType == 1) && row[i].id != 0) { // 普通座位和残疾人座位
					colOrder++;

					if (row[i].seatStatus == 0) { // 普通座位可售
						col = pui.createEl('li', 'ui-seats-col ui-seats-white');
					} else if (row[i].seatStatus == 100) { // 普通座位已售
				...
				} else if ((row[i].seatType == 2 || row[i].seatType == 3) && row[i].id != 0) { // 情侣座
					colOrder++;

					if (row[i].seatStatus == 0) { // 情侣座可售
						col = pui.createEl('li', 'ui-seats-col-couple ui-seats-couple-white');
					} else if (row[i].seatStatus == 100) { // 情侣座已售
	...
	Seats.prototype.setStatus = function(row_col, status) {
		if (Array.isArray(row_col)) {
			row_col.forEach(v => {
				pui.removeClass(this.getSeat(v), defaultConfig.hasSelect + ' ' + defaultConfig.coupleHasSelect);
				pui.addClass(this.getSeat(v), defaultConfig[status]);
			})
		}
